            ...
            if (line.contains("\"attr\":[")) {
                try {
                    ...
                    if (items.isJsonArray()) {
                        Set<String> softIds = new HashSet<String>();
                        JsonArray jsonArray = items.getAsJsonArray();
                        if (jsonArray.size() != 0) {
                            for (int i = 0; i < jsonArray.size(); i++) {
                                int id = new Float(jsonArray.get(i).getAsFloat()).intValue();
                                softIds.add(id + "");
                            }
                ...
                try {
                    ...
                    if (items.isJsonArray()) {
                        ...
                        if (jsonArray.size() != 0) {
                            for (int i = 0; i < jsonArray.size(); i++) {
                                String id = jsonArray.get(i).getAsBigDecimal().stripTrailingZeros().toPlainString();
                                softIds.add(id);
                            }
                            for (String id : softIds) {
                                sb.append(id).append("#");
